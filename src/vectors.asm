...
; Soft crash handler removed to save ROM space (Yeah, sorry)
Crash::
...
; STAT handler
; This overflows all remaining vectors to avoid jumping elsewhere, because
;  we don't need the other interrupts and it's a little faster this way.
STATVector:
    push    af
    push    hl
    ...
    ldh     a, [hRasterAddr]
    ld      l, a
    add     4                   ; store the pointer for the next interrupt here
    ldh     [hRasterAddr], a    ;  so the interrupt can return faster and avoid
                                ;  a bug that only occurs on CGB.
    ld      h, HIGH(wRasterLookup)

    ; Wait through the full scanline for VRAM access

    ; We can either check STAT to wait for HBlank (6 bytes), or burn cycles
    ;  to drop into a 'safe' point in HBlank (5 bytes), which gives us slightly
    ;  more time on the tail end, allowing for more VRAM-safe writes elsewhere.
    ; Note: Since we're packing this into vector ROM the size difference isn't
...
;     ; The cycle checking approach SEEMS to sometimes cause raster quirks,
;     ;  likely because it needs to loop longer in those cases. That is, we're
;     ;  not counting to the worst case.

;     ; TODO: Verify worst case?
; :   ldh     a, [rSTAT]
;     and     STATF_BUSY
;     jr      nz, :-  
...
VBlankHandler:
    ...
    ; during which it's running, which is fine because we don't intentionally
    ; show sprites over the first 8 lines (the HUD).
    ldh     a, [hOAMHigh]
    and     a
    jr      z, .noOAMTransfer
    call    hOAMDMA
...
.noOAMTransfer
    ...
    ld      hl, wRasterLookup
    ld      a, [hli]
    ldh     [rLCDC], a  ; Note: This is used instead of the hLCDC shadow register
    ld      a, [hli]
    ldh     [rWX], a

    ld      a, [hli]    ; setup for first full entry
    ldh     [rLYC], a
    ld      a, l
    ldh     [hRasterAddr], a
    pop     hl

    ; TODO: Perhaps populate the entire first row here from the lookup?
    ; (add new default first row, so we can move the window for the status screen)
    ; Initial window position for HUD (can't use the raster lookup because we need it set for line 0)
    ;ld      a, 7
    ;ldh     [rWX], a
    ...
    ; Normally these'd just be called here regardless to ensure consistent
    ;  timing, but in order to ensure the raster table is updated in time during
    ;  gameplay, these updates are performed later in the frame instead of here.
    ldh     a, [hVBlankUpdateAudio]
    or      a
    jr      z, .noAudioUpdates
    push    bc
    push    de
...
.lagFrame
    ...
    ; Occasionally a STAT check may have occurred just before VBlank fired,
    ;  and if the music+sfx updates take long enough we might return into
    ;  the middle of mode 3, no longer with VRAM access. To avoid this, we
    ;  wait here until VRAM is accessible.
:   ldh     a, [rSTAT]
    and     STATF_BUSY
    jr      nz, :-
...
; If this is not zero, call the music and sound effect update routines during VBlank. 
hVBlankUpdateAudio:: db
